...
export default function HeroCoin({
  ...
  const handleMine = () => {
    if (burst) return;
    setBurst(true);
    if (typeof onMine === 'function') onMine();
  ...
  useEffect(() => {
    ...
    setBurst(true);
    const timer = window.setTimeout(() => setBurst(false), 500);
  ...
  return (
    <section className="hero-coin">
      ...
      <button
        type="button"
        aria-label="Mine MAI"
        className={`hero-coin-button ${burst ? 'burst' : ''}`}
        onClick={handleMine}
      >
      ...
      {burst && (
        <>
          ...
          <div className="reward-pop">+MAI</div>
        </>
      )}
      ...
      <div className="coin-platform">
        ...
        <div className="platform-ring p-three" />
        <div className="platform-center"><span /></div>
      </div>

      <div className="coin-balance">
        <span>MAI BALANCE</span>
        <strong>
          {Number(balance).toFixed(4)} <small>MAI</small>
        </strong>
      </div>
      ...
      <div className="coin-rate">
        <span>⚡</span>
        <div>
          <small>AUTO MINING</small>
          <strong>+{Number(miningRate).toFixed(8)} MAI / SEC</strong>
        </div>
      </div>
    </section>
  );
